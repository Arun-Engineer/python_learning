...
class BugTracker:
    ...
    def next_bug_id(self) -> int:
        if not self.bugs:
            logger.info(f"if not present in bugs")
            return 1
        logger.info(f"{max(bug.id for bug in self.bugs) + 1} this is the next bug id")
    ...
    def add_bug(self, title: str, severity: int, status: str, team: str) -> Bug:
        ...
        logger.info("adding bugs to the list")

        self.bugs.append(new_bug)

        logger.info("Saving bugs in %s", self.filename)
